...
def Rdr(conn1, conn2, e, serie, data=None):
    if data:          #transmitiendo a proxy server
        ...
        while True:
            try:
                data = conn2.recv(4196)   #ACK
                #data2 =  conn2.recv(2048) #eco 
            except:
                data = None
                break
            if not data: break
                ...
                if int(perdida[1])>0:
                    while True:
                        try:
                            ...
                            cosas = conn2.recv(4196)   #no llegó el ACK de ACKs?? => siguen llegando msjs "DX______"
                        except:
                            cosas=None
                            break
                        if not cosas: break
