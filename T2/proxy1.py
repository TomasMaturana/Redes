...
perdida="L0"  #probabilidad de pérdida de paquetes ##
#####################################################
...
def Rdr(conn1, conn2, e, serie, data=None):
    if data:          #transmitiendo a proxy server
        ...
        while True:
            try:
                data = conn2.recv(1400)   #ACK
                data2 =  conn2.recv(1400) #eco 
            except:
                data = None
                break
            if not data: break
                ...
                if int(perdida[1])>0:
                    while True:
                        try:
                            ...
                            conn2.settimeout(3*int(perdida[1]))
                            cosas = conn2.recv(1400)   #no llegó el ACK de ACKs?? => siguen llegando msjs "DX______"
                        except:
                            cosas=None
                            break
                        if not cosas: break
